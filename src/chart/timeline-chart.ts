...
const cut = (datasets: Fragment[]) => {
    const { currentTime } = document.getElementById('video') as any
    if (!datasets || !currentTime) return;
    const fragment = datasets[0].data.find(frag => {
                        ...
                        plugins: [
                            {
                                afterRender: (chart) => {
                                    ...
                                    document.getElementById('cut').addEventListener('click', () => cut(datasets))
    ...
    private click(event: MouseEvent) {
        ...
        if (element.length && chart.data.datasets) {
            ...
            if (self.hls?.media) {
                const scale = this.chartScales[X_AXIS_SECONDS];
                const pos = Chart.helpers.getRelativePosition(event, chart);
                const newCurrentTime = scale.getValueForPixel(pos.x)
                console.log({pos, newCurrentTime})
                self.hls.media.currentTime = newCurrentTime;
                drawCut(30)
            }
        }
    }
            ...
            datasets.push(
                datasetWithDefaults({
                    url: Array.isArray(level.url)
                        ...
                        : level.url,
                    trackType: 'level',
                    // backgroundColor: 'green', // тут красится контейнер с фрагментами
                    borderColor,
                    level: index,
                })
            );
...
const drawCut = (pos: number) => {
    console.log('DRAW')
    const canvas = document.getElementById('timeline-chart') as any;
    const ctx = canvas.getContext('2d');
    ctx.lineWidth = 1;
    ctx.strokeStyle = 'red'
    ctx.beginPath();
